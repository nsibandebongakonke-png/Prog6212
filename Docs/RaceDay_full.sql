-- ============================================
-- RaceDay Database — Schema + SELECT Statements
-- ============================================
...
CREATE TABLE event (
    ...
    distance VARCHAR(50), -- or DECIMAL(5,2) but could be text
    event_type VARCHAR(50),
    banner_image VARCHAR(255),
    FOREIGN KEY (organiser) REFERENCES user(user_id)
);
...
CREATE TABLE enrolment (
    ...
    enrol_date DATETIME DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE supporting_document (
    ...
    status VARCHAR(20) DEFAULT 'pending',
    FOREIGN KEY (enroll_id) REFERENCES enrolment(enroll_id)
);
 
CREATE TABLE race_result (
    result_id INT PRIMARY KEY AUTO_INCREMENT,
    enroll_id INT NOT NULL,
    finish_time TIME, -- could be TIME or DATETIME? maybe time only
    position INT,
